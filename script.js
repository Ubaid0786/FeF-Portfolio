const contactHeading = document.querySelector(".contact-heading");
     const contactDescription = document.querySelectorAll(".contact-description");
     ...
     setTimeout(() => {
       contactDescription.forEach((description, index) => {
 ...
 let currentCardIndex = 0;
       ...
       function autoToggleNextCardContent() {
         ...
         setTimeout(autoToggleNextCardContent, 30000); // Automatically change after 5 seconds
       }
     ...
     // Resize event listener to close the mobile menu on window resize
     ...
     // Fix the navbar and burger on scroll
     ...
     // Hide the burger icon for large screens
     window.addEventListener('resize', () => {
       if (window.innerWidth > 768) {
         burger.style.display = 'none';
         navLinks.classList.remove('nav-active'); // Close the mobile menu on resize
       } else {
     ...
     // Function to reveal the elements with delay for live-writing effect
     ...
     // JavaScript for dynamic introduction with typing animation
 ...
 function typeText(text, element) {
   let charIndex = 0;
   const typingSpeed = 100; // Adjust typing speed here (lower value means faster typing)
 ...
 function togglePoem(technology) {
   ...
   if (activePoem !== null && activePoem !== poem ) {
 ...
 function typePoem(poem) {
  ...
  type();
     ...
     // Projects data (You can replace these placeholder images and descriptions with your actual projects)
     ...
     function showMoreProjects() {
       // Add code here to show more projects or navigate to another page
     }
